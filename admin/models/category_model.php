...
//      Подготовка данных, чтобы просмотреть все дерево
    ...
    function editCategory($id, $name) {
        ...
        $res = $this->db_tree->Execute($sql);
        if (0 == $res->RecordCount()) {
            return false;
        }
        
        $sql = $this->db_tree->GetUpdateSQL($res, $name);
        if (!empty($sql)) {
            $res = $this->db_tree->Execute($sql);            
        }
        return true;
    ...
    function moveCategory($type, $id, $post){
        ...
        if($type=='form'){
            ...
            $this->objTree->Branch($item[0]['category_id'], array('category_id', 'category_name'), array('and' => array('category_level = ' . $current_category[2])));
            $dataForm=array();
            foreach ($this->objTree->NextRow() as $item) {  
                $dataForm[0][]=$item;
            }
