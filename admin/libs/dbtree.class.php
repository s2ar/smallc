...
class dbtree {
    ...
    */
    function GetNodeInfo($section_id, $cache = FALSE) {
        ...
        if (FALSE === DB_CACHE || FALSE === $cache || 0 == (int)$cache) {
            ...
            $res = $this->db->query((int)$cache, $sql);
        }
        if (FALSE === $res) {
            $this->ERRORS[] = array(2, 'SQL query error.', __FILE__ . '::' . __CLASS__ . '::' . __FUNCTION__ . '::' . __LINE__, 'SQL QUERY: ' . $sql, 'SQL ERROR: ' . $this->db->ErrorMsg());
            $this->ERRORS_MES[] = extension_loaded('gettext') ? _('internal_error') : 'internal_error';
    ...
    */
    function Insert($section_id, $condition = '', $data = array()) {
        ...
        $data[$this->table_id] = $this->GenID($this->table . '_seq', 2);
    ...
    */
    function Full($fields, $condition = '', $cache = FALSE) {
        ...
        if (FALSE === DB_CACHE || FALSE === $cache || 0 == (int)$cache) {
            ...
            $res = $this->db->query((int)$cache, $sql); // TODO Управление кешированием
        }
        if (FALSE === $res) {
            $this->ERRORS[] = array(2, 'SQL query error.', __FILE__ . '::' . __CLASS__ . '::' . __FUNCTION__ . '::' . __LINE__, 'SQL QUERY: ' . $sql, 'SQL ERROR: ' . $this->db->ErrorMsg());
            $this->ERRORS_MES[] = extension_loaded('gettext') ? _('internal_error') : 'internal_error';
    ...
    */
    ...
    function GenID($seqname, $start) {
       ...
       // return mysql_insert_id($this->conn);
        $lastID = $this->db->selectCell('SELECT LAST_INSERT_ID() as ID FROM '.addslashes($seqname));
        return $lastID;
    }
    ...
    */
    function GetInsertSQL($res_sql, $data) {
        if (empty($data)) {
            return '';
        }
        preg_match_all("~FROM\s+([^\s]*)~", $res_sql, $maches, PREG_PATTERN_ORDER);
        if (!isset($maches[1][0])) {
            return '';
        } else {
            $table = $maches[1][0];
    ...
    * @return string - Complete SQL query or empty string
    */
    function GetUpdateSQL($res_sql, $data) {
        if (empty($data)) {
            return '';
        }
        preg_match_all("~FROM\s+([^\s]*)~", $res_sql, $maches, PREG_PATTERN_ORDER);
        ...
        preg_match_all("~(WHERE\s+.*)~is", $res_sql, $maches, PREG_PATTERN_ORDER);
        if (!isset($maches[0][0])) {
            return '';
        } else {
            $where = $maches[0][0];
